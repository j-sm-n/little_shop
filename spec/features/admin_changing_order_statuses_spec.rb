...
RSpec.feature "Admin changing order statuses", type: :feature do
  ...
  scenario "filter orders by status" do
    ...
    expect(page).to have_link("Order #1", href: order_path(order_paid_1))
    expect(page).to have_link("Order #2", href: order_path(order_paid_2))

    visit admin_dashboard_path

    click_on "Completed Orders"

    expect(page).to have_link("Order #1", href: order_path(order_completed))
  end
